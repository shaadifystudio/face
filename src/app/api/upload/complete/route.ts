...
export async function POST(req: Request) {
  try {
    ...
    for (const photo of photos) {
      const { data: signed, error: signedError } = await supabase.storage.from(BUCKET).createSignedUrl(photo.storage_path, 60 * 60);
      if (!signedError && signed?.signedUrl) verified.push(photo.id);
    }

    if (!verified.length) return NextResponse.json({ error: 'No uploaded files could be verified.' }, { status: 400 });
    ...
    if (workerUrl && workerSecret) {
      ...
      await fetch(`${workerUrl.replace(/\/$/, '')}/enqueue`, {
        ...
        body: JSON.stringify({ wedding_id: weddingId, photo_ids: verified }),
      }).catch(() => undefined);
    }
    ...
    return NextResponse.json({
      ok: true,
      queued: verified.length,
      status: weddingStatus,
      aiReady: Boolean(workerUrl && workerSecret),
      bucket: BUCKET,
    });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Could not finalize upload.' }, { status: 500 });
  }
}