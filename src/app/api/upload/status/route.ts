...
export async function GET(req: Request) {
  ...
  try {
    const { client: supabase } = getRequestSupabase(req); const { data: wedding, error: weddingError } = await supabase.from('weddings').select('id,status,photo_count,face_count,studios!inner(owner_id)').eq('id', weddingId).single();
